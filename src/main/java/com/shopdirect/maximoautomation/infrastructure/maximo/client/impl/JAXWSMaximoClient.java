...
public class JAXWSMaximoClient implements MaximoClient {
    ...
    private MXISWOCHANGEPortType createAndConfigureClient(String maximoUrl) {
        ...
        requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, maximoUrl);
    ...
    public String createChange(BuildInfo buildInfo) {
        ...
        try {
            ...
            String changeId = response.getWOCHANGEMboKeySet().getWOCHANGE().get(0).getWONUM().getValue();
            LOGGER.info("Maximo Change created with ID {}", changeId);
    ...
    public String fallbackCreateChange(BuildInfo buildInfo) {
        LOGGER.warn("Resuming with fallback for build info {}", buildInfo);
        return null;
    }
    ...
    private CreateMXISWOCHANGEType createMXISWOCHANGEType(BuildInfo buildInfo) throws DatatypeConfigurationException {
        ...
        wochange.setPLUSPCUSTOMER(createMXStringType(maximoChangeRequestConfig.getPlusPCustomer()));
        wochange.setLOCATION(createMXStringType(maximoChangeRequestConfig.getLocation()));
        wochange.setASSETNUM(createMXStringType(maximoChangeRequestConfig.getAssetNum()));
